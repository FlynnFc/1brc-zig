...
// WeatherStation represents a weather station with an ID and mean temperature.
type WeatherStation struct {
	ID              string
	MeanTemperature float64
}

// Measurement generates a temperature measurement based on a Gaussian distribution.
func (ws WeatherStation) Measurement() float64 {
	measurement := ws.MeanTemperature + rand.NormFloat64()*10
	measurement = math.Round(measurement*10) / 10.0
	return measurement
}
...
func main() {
	...
	measurementFile := filepath.Join(".", "measurements.txt")
	...
	for i := 0; i < size; i++ {
		...
		_, err := writer.WriteString(fmt.Sprintf("%s;%.1f\n", station.ID, measurement))
		if err != nil {
	...
	if err := writer.Flush(); err != nil {
		fmt.Printf("Error flushing data to file: %v\n", err)
		os.Exit(1)
	}
